...
| **Monitoring** | MLflow (training) + Prometheus + Grafana (serving) + JSON access logs |
...
└────────────┘   └──────────────────────┘   │ 60-day sliding windows  │
                          ...
                          └─────────────────────┬────────────────────────┘
                                      ...
                                      └──────────┬───────────┘
                                                 ▼
                          JSON access logs · Prometheus → Grafana
```
...
The compose stack ships with two extra services that provide live observability:

| Service          | Host port | URL                       | Purpose                                         |
| ---------------- | --------- | ------------------------- | ----------------------------------------------- |
| `baba-prometheus`| **9090**  | `http://localhost:9090`   | Time-series database. Scrapes `baba-api:/metrics` every 15 s. 15-day retention. |
| `baba-grafana`   | **3001**  | `http://localhost:3001`   | Dashboards. Default login `admin` / `admin`. The **BABA Overview** dashboard is auto-provisioned. |

Open Grafana → Dashboards → **BABA** folder → **BABA Overview**. Four rows:

1. **Service health**: request rate by route, error rate %, model loaded YES/NO, HTTP latency p50/p95/p99.
2. **Model serving**: predictions per minute, pure inference p95, distribution heatmap of predicted prices.
3. **Input drift**: heatmaps of input-window mean and std — early warning that the data feeding the model has shifted away from the training distribution.
4. **Training observability**: time since last `/train`, last test metrics (MAE/RMSE/MAPE), total training runs.

Generate some traffic to see panels light up:

```powershell
1..30 | ForEach-Object { curl.exe -s -X POST http://localhost:8010/predict/latest | Out-Null }
```

Wait ~15 s (one scrape interval) and refresh the dashboard.

> **What's monitored, in one sentence:** standard HTTP signals (rate, errors, latency) **plus** ML-specific signals (prediction count, pure inference time, predicted-value distribution, input-window drift, freshness of the trained model). The full metric list is in `src/api/metrics.py`.

### 6. Frontend: Streamlit UI

For an end-to-end UI that talks to the API without `curl` or Swagger, open **http://localhost:8501**. The page has:

- A live **health indicator** in the sidebar that polls `/health` every 15 s.
- A hero card with the **next-day predicted close**, fetched on page load with a manual **↻ Refresh** button.
- A **Last training metrics** card (MAE / RMSE / MAPE / trained-at), pulled by parsing Prometheus' `baba_last_train_metric` gauges.
- A **Train a new model** form with full hyperparameter control (epochs, units, batch_size, learning_rate, dropout, lookback, patience, seed). Submits to `POST /train` and blocks for ~1 minute.
- A **Manual prediction** form: paste 60 closing prices, get a prediction back from `POST /predict`. Useful for backtesting / counterfactual demos.
- Sidebar quick-links to MLflow, Grafana, Prometheus, and the API docs.

| Service          | Host port | URL                       |
| ---------------- | --------- | ------------------------- |
| `baba-streamlit` | **8501**  | `http://localhost:8501`   |
...
**Prometheus exposition format** (text). Scraped by the `baba-prometheus` service every 15 s; visualised in the **BABA Overview** Grafana dashboard.

Includes both standard HTTP metrics (`http_requests_total`, `http_request_duration_seconds`, process collectors) auto-generated by `prometheus-fastapi-instrumentator`, and custom domain metrics defined in `src/api/metrics.py`:

| Metric | Type | What it captures |
| --- | --- | --- |
| `baba_predictions_total{endpoint}` | counter | One per successful prediction. |
| `baba_inference_duration_seconds{endpoint}` | histogram | Pure TF forward-pass time. |
| `baba_predicted_close_usd` | histogram | Distribution of predicted prices. |
| `baba_model_loaded` | gauge | 1 if predictor is in memory, else 0. |
| `baba_input_window_mean_usd` | histogram | Mean of each incoming 60-day window (drift watch). |
| `baba_input_window_std_usd` | histogram | Std of each incoming 60-day window. |
| `baba_train_runs_total` | counter | Successful `/train` calls. |
| `baba_train_duration_seconds` | histogram | Wall-clock training duration. |
| `baba_last_train_timestamp_seconds` | gauge | When the last training finished. |
| `baba_last_train_metric{metric}` | gauge | MAE / RMSE / MAPE from the last training. |

Sample raw output (truncated):

```
# HELP baba_predictions_total Total predictions served.
# TYPE baba_predictions_total counter
baba_predictions_total{endpoint="/predict/latest"} 12.0
baba_predictions_total{endpoint="/predict"} 3.0
# HELP baba_inference_duration_seconds Pure TF forward-pass time, excluding request overhead.
# TYPE baba_inference_duration_seconds histogram
baba_inference_duration_seconds_bucket{endpoint="/predict/latest",le="0.005"} 0.0
...
```
...
Real-time metrics (request rate, latency percentiles, ML-specific signals) live in **Prometheus**, scraped from `/metrics` every 15 s and visualised in the **BABA Overview** Grafana dashboard. See the [Observability section](#5-observability-prometheus--grafana) above for details.
...
Run the pytest suite **inside the running API container**, no host Python required:

```powershell
# Stack must be up: docker compose up -d --build
...
- FastAPI routes via `TestClient`: `/health`, `/predict` happy + validation errors, `/train` smoke + validation, `/metrics` Prometheus format (`tests/test_api.py`)

Tests run in ~30 seconds without hitting yfinance or training a real model (they use a synthetic sine-wave dataframe; see `tests/conftest.py`). `pytest`, `pytest-asyncio` and `httpx` are baked into the runtime image so this works out of the box.
